...
// WCAE: limit packet delivery to the web/proxy ports selected for the WeChat rule.
// The original kernel driver remains byte-for-byte unchanged and signed.
static char wcae_port_filter[768] = "(tcp.DstPort == 80 or tcp.DstPort == 443)";
volatile BOOL wcae_relay_ready = FALSE;
PROXYBRIDGE_API BOOL Wcae_ConfigureCapture(const char *ports, UINT16 relay_port)
{
    char buffer[128], *context = NULL, *value;
    if (running || !ports || strlen(ports) >= sizeof(buffer) || relay_port < 1024) return FALSE;
    g_local_relay_port = relay_port;
    strcpy_s(buffer, sizeof(buffer), ports);
    strcpy_s(wcae_port_filter, sizeof(wcae_port_filter), "(");
    value = strtok_s(buffer, ";", &context);
    int count = 0;
    while (value)
    {
        char *end = NULL, entry[64]; long port = strtol(value, &end, 10);
        if (!end || *end || port < 1 || port > 65535 || count >= 10) return FALSE;
        snprintf(entry, sizeof(entry), "%stcp.DstPort == %ld", count ? " or " : "", port);
        strcat_s(wcae_port_filter, sizeof(wcae_port_filter), entry); count++;
        value = strtok_s(NULL, ";", &context);
    }
    strcat_s(wcae_port_filter, sizeof(wcae_port_filter), ")");
    return count > 0;
}
...
PROXYBRIDGE_API BOOL ProxyBridge_Start(void)
{
    ...
    for (int i = 0; i < 100 && !wcae_relay_ready; i++)
    {
        if (WaitForSingleObject(proxy_thread, 10) == WAIT_OBJECT_0) break;
    }
    if (!wcae_relay_ready)
    {
        running = FALSE;
        WaitForSingleObject(proxy_thread, 1000);
        CloseHandle(proxy_thread); proxy_thread = NULL;
        log_message("WCAE relay start failed"); return FALSE;
    }
    ...
    // WCAE does not need DNS snooping or any UDP interception. All other applications'
    // packets at these ports are re-injected unchanged by the process rule engine.
    snprintf(filter, sizeof(filter),
        "not impostor and tcp and (((outbound or loopback) and %s) or tcp.SrcPort == %d or tcp.DstPort == %d)",
        wcae_port_filter, g_local_relay_port, g_local_relay_port);
